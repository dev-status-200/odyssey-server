...
routes.post("/createClientInBulk", async(req, res) => {
    ...
    const createChildAccounts = (list, name) => {
        let result = [];
        list.forEach((x)=>{
            result.push({title:name+ `${x.title=="ACCOUNT RECEIVABLE"?" RECEIVABLE":" PAYABLE"}`, ParentAccountId:x.id})
        })
        return result;
    }
    const createAccountList = (parent, child, id) => {
    ...
    try {
        obj.forEach(async(l)=>{
            ...
            await Client_Associations.bulkCreate(createAccountList(accounts, accountsList, result.id)).catch((x)=>console.log("===========3===========", x))
